...
However, we quickly realized that this project was ambitious. At this stage of our academic journey, our technical knowledge of econometrics is not sufficient to perform a solid panel regression that would address all the needs of our dataset. There are notions that are beyond our current grasp and knowledge such as how to correct serial correlation in panel data, to deal with potential non-linearity or the use of more complex models such as dynamic models. Nevertheless, we used this assignment as a training ground to familiarize ourselves with the procedures for performing an econometrics analysis on panel data.

\<br\>

After looking into various sources, our final procedure drew inspiration from Altaf Ali (2017). The codes and the process were also influenced by a document by Torres-Reyna (2010) for Princeton University, as well as insights from articles by Schmidheiny (2023) and Wagle (2021). Bourbonnais (2020) also played a role in our understanding part of the theory behind the use of panel data.
...
An early step consists to analyze which technis fits the most the dataset. The F Test for Individual and/or Time Effects of the plm package (pFtest) enable to arbitrate between the used of a fixed effect model a simple OLS. The decision rule is that if the p-value is below the significance level of 0.05, the fixed effect model is preferred. Here our p-value is, then we will prefered the fixed model.
...
With a p value below 0.05, the XXXX test support the hypothesis of the use of time-fixed effect.
...
f_eff_time_2<-plm(entrées~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2, index=c("departement_code", "year"), model="within", effect = "time")

#PLM test
plmtest(f_eff_time_2, effect="time")
...
f_eff_ind_2<-plm(entrées~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2, index=c("departement_code", "year"), model="within", effect = "individual")

# PLM test
plmtest(f_eff_ind_2, effect="individual")
```
