...
The movie theaters market experienced growth relatively quickly. According to Muhlberger (2013) it already became a mass leisure activity in Franc in 1920s but also in the USA and numerous European countries. However, it fully blossomed in the Hexagone a decade later. The period between 1929 and 1957 is often described as a golden age for cinema in France. Movie theaters attendance reached its zenith, with peaks beyond 400 millions entries. In a report for the French Senate, Thiolliere and Ralite (2003) link this fast period with the introduction of talking films.

<br>

The importance of cinema during this period maybe underestimated by our contemporary eyes. However, the two senatorial rapporteurs explain that :

" until the advent of the 'petite lucarne' (i.e. television) in households, cinema had a monopoly on the broadcasting of images. People frequented the theater, not to see a film, but rather to watch a program consisting of one or more short films, news, the trailer for the next film and, after the intermission and advertising, of the 'feature film'."
...
The map below show the evolution of the attendance by department and year in France. Each year show the total growth rate between this year and the 1966 year. One can that the majority of the deportment are not spare by the decline. However, the trend is not homogeneous by department. Some has a periodnone of department are spare at the end. The department of the north-east of the Hexagon knew a stronger degrowth of their attendance. At the opposite some departments of Ile-de-France and of the south-east of the territory had a better resilience even knowing significant growth phases.
...
The Haussman test continue our analysis by discerning the most suitable model between the fixed-effect and the random model. The p-value being below the significance level of 0.05, the fixed effect model is favored.
...
Having selected the fixed effect model, the next step was to identify if the addition time-fixed effects improve the model's quality.

With p-values below 0.05, both Breusch-Pagan and Honda tests support the introduction of fixed effect.
...
We then used a two-ways model combining time-fixed and individual effects.
...
However, our panel data encounter several issues. Firstlyn our model also suffer from serial correlation. The Breusch-Godfrey/Wooldridge indicate a p-value below the significance level of 0.05. To correct this issue, both Altaf and Torres-Reyna propose t the use of Arellano's technique as we will see in the next paragraph.
...
The second challenge of our ecometrics analysis was the presence of heteroscedasticity in the model.

```{r #Test_for_heteroscedasticity}
bptest(entrées~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data = Econ_data_v2, studentize=F)
```

Nevertheless, heteroscedasticity if a common issue in ecometric analysis and solutions has been presented to tackle it. The use of the **vcovHC** function from the **sandwich** package computes heteroskedasticity-consistent covariance estimators using White and Arellano method (White 1980, 1984; Arellano, 1987). Moreover, this function can be used to correct simultaneously both heteroskedasticity and serial correlation with ' **method = "arellano"** '.

#### Test for cross-sectional dependence

A final issue was the presence of cross-sectional dependance as we can see on the Breusch-Pagan and Pesaran tests below.
...
The utilization of Beck and Katz method is proposed by Altaf as a solution to tackle this issue. To use this method, we will forego the utilization of the **vcovHC** function but do a similar work with the **vcovBK** one, which also enable to correct cross-sectional dependence.

### Discussion

```{r Results_before_correction}
summary(f_eff_2ways_2)
```
...
k_model <- length(coef(f_eff_2ways_2))
