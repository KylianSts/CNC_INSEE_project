...
Re_graph2 <- recette_nat_withou_inf %>% 
  filter(year < 1993) %>% 
  ggplot(aes(x = year, y = recette_tot)) +
  geom_line(color="skyblue") +
  scale_y_continuous(labels = scales::number_format())+ 
...
```{r}
Re_graph4 <- recette_nat_withou_inf %>% 
  filter(year > 1992) %>% 
  ggplot(aes(x = year, y = rme_nat_without_inflation)) +
  geom_line(color="skyblue") +
  scale_y_continuous(labels = scales::number_format())+ 
...
pop_dep_old <- insee_data_clean %>%
  filter(AGE_GROUP == '60-74' | AGE_GROUP == '75 and more') %>%
  group_by(departement_code, year) %>%
  summarise(population_dep = sum(POND2))

pop_fr_old<- insee_data_clean %>%
  filter(AGE_GROUP == '60-74' | AGE_GROUP == '75 and more') %>%
  group_by(year) %>%
  summarise(population_fr = sum(POND2))

pop_dep_old <- left_join(pop_dep_old,pop_fr_old, by= 'year') %>%
  mutate(per_pop_dep_old = population_dep/population_fr * 100)

base_year <- pop_dep_old %>%
  group_by(departement_code) %>%
  filter(year == 1990) %>%
  select(per_pop_dep_old, departement_code) %>%
  rename(base=per_pop_dep_old)

pop_dep_old <- left_join(pop_dep_old, base_year, by = "departement_code") %>%
  mutate(per_pop_dep_old_change = (per_pop_dep_old / base -1)*100)
```
...
merged_data <- merge(france_shape, pop_dep_old, by = "departement_code")

ggplot(merged_data, aes(fill = per_pop_dep_old_change)) +
  geom_sf() +
  scale_fill_viridis_c(option = "H") + 
  labs(title = "Population Map of France", fill = "per_pop_dep_old_change") +
  theme_minimal() +
  facet_wrap(~year, ncol = 2)
```
