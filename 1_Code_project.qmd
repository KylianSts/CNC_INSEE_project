...
att_dep_1 <- cnc_data_clean %>%
  ...
  filter(year == 1966 | year == 1970 | year == 1974 | year == 1978 | year == 1982 | year == 1986 |year == 1990 | year == 1992) %>%
...
att_dep_2 <- cnc_data_clean %>%
  group_by(departement_code, year) %>%
  filter(year == 1993 | year == 1996 | year == 2000 | year == 2004 | year == 2008 | year == 2012 |year == 2016 | year == 2019) %>%
  summarise(attendance_dep = sum(entrées))
...
Classement des établissements " Art et Essai "
Le classement Art et Essai des lieux de projection cinématographiques a fait l'objet d'une réforme qui s'applique depuis 2002.
...
```{r}
attAE_dep_now <- cnc_data_clean %>%
  group_by(departement_code) %>%
  filter(year == 2019) %>%
  summarise(attendanceAE_dep = sum(entréesAE))

attAE_fr <- att_dep_2 %>%
  filter(year == 2019) %>%
  select(departement_code, attendance_dep)

attAE_dep_now <- left_join(attAE_dep_now,attAE_fr, by= 'departement_code') %>%
  mutate(part_AE = attendanceAE_dep/attendance_dep * 100)
```
...
merged_data <- merge(france_shape, attAE_dep_now, by = "departement_code")

ggplot(merged_data, aes(fill = part_AE)) +
  geom_sf() +
  scale_fill_viridis_c(option = "H") + 
  labs(title = "Where do we consume the most arthouse cinema ?", fill = "Part of the arthouse cinema in totale attendance") +
  theme_minimal() +
  annotation_scale()+
  annotation_north_arrow(style = north_arrow_nautical, location = "tr", which_north = "true")+
  theme(legend.position = "bottom",
...
#For fixed effect
f_eff <-plm(y ~ x1, data=Panel, index=c("dep", "year"), model="within") > summary(fixed)

#For random effect
r_eff <-plm(y ~ x1, data=Panel, index=c("dep", "year"), model="random") > summary(random)

#Haussman test to determine between fixed and random effect
phtest(f_eff, r_eff)

#Test r_eff-sectional dependence
pcdtest(r_eff, test = c("lm"))
pcdtest(r_eff, test = c("cd"))
 
# Test time-fixed effects.
r_eff_time<-plm(y ~ x1 + factor(year), data=Panel, index=c("country", "year"), model="random")
pFtest(r_eff_time, r_eff)
