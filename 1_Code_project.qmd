...
calculate_growth_rate <- function(data, start_year, end_year) {
  ...
  growth_rate <- subset_data %>%
    reframe(Growth = (Attendance[2:n()]/Attendance[1:(n()-1)] - 1) * 100)%>%
    filter(!is.na(Growth)) %>%
    pull(Growth) %>%
    round(digits = 2)
...
Av_Growth_rate_1966_1992 <- Attendance_nat %>% 
  ...
  select(-Growth) %>% 
  mutate(Growth = ((Attendance / c(NA,Attendance[1:(n()-1)]))^(1/nrow_1966_1992) - 1) * 100) %>%
  filter(!is.na(Growth)) %>% 
  pull(Growth) %>%
  round(., digits=1)
```
...
### Decomposition en deux periodes
...
#For fixed effect
f_eff_2_1 <-plm(entrées ~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2_1, index=c("departement_code", "year"), model="within")
summary(f_eff_2_1)

#For random effect
r_eff_2_1 <-plm(entrées ~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2_1, index=c("departement_code", "year"), model="random") 
summary(r_eff_2_1 )

#Haussman test to determine between fixed and random effect
phtest(f_eff_2_1, r_eff_2_1)

#Test r_eff-sectional dependence
pcdtest(f_eff_2_1, test = c("lm"))
pcdtest(f_eff_2_1, test = c("cd"))
 
# Test time-fixed effects.
f_eff_time_2_1<-plm(entrées~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina + factor(year), data=Econ_data_v2_1, index=c("departement_code", "year"), model="within")
pFtest(f_eff_time_2_1, f_eff_2_1)
plmtest(f_eff_2_1, c("time"), type=("bp"))
...
#Test for unit roots
 Panel.set<- plm.data(Econ_data_v2_1, index = c("departement_code", "year")) 

 adf.test(Econ_data_v2_1$entrées, k=2)
```

#### Apres 1990 (non inclus)

```{r}
Econ_data_v2_2<- Econ_data_v2 %>% filter(year > 1990)

#For fixed effect
f_eff_2_2 <-plm(entrées ~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2_2, index=c("departement_code", "year"), model="within")
summary(f_eff_2_2)

#For random effect
r_eff_2_2 <-plm(entrées ~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2_2, index=c("departement_code", "year"), model="random") 
summary(r_eff_2_2 )

#Haussman test to determine between fixed and random effect
phtest(f_eff_2_2, r_eff_2_2)

#Test r_eff-sectional dependence
pcdtest(f_eff_2_2, test = c("lm"))
pcdtest(f_eff_2_2, test = c("cd"))
 
# Test time-fixed effects.
f_eff_time_2_2<-plm(entrées~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina + factor(year), data=Econ_data_v2_2, index=c("departement_code", "year"), model="within")
pFtest(f_eff_time_2_2, f_eff_2_2)
plmtest(f_eff_2_2, c("time"), type=("bp"))
...
#Test for unit roots
 Panel.set<- plm.data(Econ_data_v2_2, index = c("departement_code", "year")) 
...
# Modèle économétrique avec uniquement CSP
mvif_2 <-plm(entrées ~ Farm + Craf_sh_head + Ex_intel + Int_prof + Employees + Workers + Retirees + Ina, data=Econ_data_v2_1, index=c("departement_code", "year"), model="pooling")
...
mvif_5 <-plm(X_Y, data=Econ_data_v4_1, index=c("departement_code", "year"), model="pooling")
