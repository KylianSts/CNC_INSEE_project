...
ggplotly(Re_graph3, dynamicTicks = TRUE)
```
...
ggplotly(Att_graph4, dynamicTicks = TRUE)
```
...
ggplotly(Re_graph5, dynamicTicks = TRUE)
```
...
ggplotly(M_graph2, dynamicTicks = TRUE)
```
...
ggplotly(AE_graph2, dynamicTicks = TRUE)
```
...
```{r Creatation_of_the_needed_datasets_for the models}

```

#### Choice of the type of model

We only employed basics yechniques for econometric analysis of panel data. Three simple and standards models are often used for panel data :
