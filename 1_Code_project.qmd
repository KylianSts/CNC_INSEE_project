...
The selection of the final were performed using the Bayesian Information Criterion. We used the formula used in Yum's paper (2021) on his analysis of is an AIC for model selection for panel data models with fixed effect :  : 
$$
...
#N
N_model <- length(residuals(f_eff_2ways_2))
...
k_model <- length(coef(f_eff_2_2ways_2))
bic_value <- n_obs * log(sigma_squared) + k_model * log(n_obs)
