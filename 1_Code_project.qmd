...
The cinema market experienced fast growth. According to Muhlberger (2013), in 1920s, cinema became a mass leisure activity in France but also in the USA and numerous European countries. In the Hexagon, cinema market fully blossomed a decade later. The period between 1929 and 1957 is often described as a golden age for cinema in France. Movie theaters attendance reached its zenith, with peaks beyond 400 millions entries. In a report for the French Senate, Thiolliere and Ralite (2003) link this fast period with the introduction of talking films.

<br>

The importance of cinema during this period maybe underestimated by our contemporary eyes. However, the two senatorial rapporteur explain that :
...
calculate_growth_rate <- function(data, start_year, end_year) {
  second_column_name <- colnames(data)[2]

  subset_data <- data %>%
    filter(year %in% c(start_year, end_year))
  
  growth_rate <- subset_data %>%
    reframe(Growth = (get(second_column_name) - shift(get(second_column_name), type = "lag")) / shift(get(second_column_name), type = "lag")  * 100) %>%
    filter(!is.na(Growth)) %>%
    pull(Growth) %>%
    round(digits = 2)
...
The map below illustrates the evolution of attendance by department and year in France. Each year reflects the total growth rate compared to the base year of 1966. It is noteworthy that the majority of the departments are affected by a decline; however, this trend is not consistent across all departments. Some regions experience periods of decline, while others remain relatively stable over time. The departments in the north-east of the Hexagon undergo a more pronounced decline in attendance. In contrast, certain departments in the Ile-de-France and the south-east of the territory exhibit better resilience, even experiencing significant growth phases.
...
Intriguingly, despite the observed drop in attendance, the revenue of the movie theater industry increases from 1966 to 1982.
...
The increase in revenue remains consistent even when adjusted for inflation, as shown in the graph below. However, the 80s and 90s are marked by an equally significant contraction.
...
The observed growth during the 70s can be partly explained by a continuous increase in the average revenue per ticket sold, indicating a rise in ticket prices.
...
The decade between 1966 and 1993 also witnessed significant structural shifts in the industry. The number of seats experienced a sharp decline during this period. While the Hexagon had 2.4 million movie theater seats in 1966, only 931,000 seats characterized the French market in 1992.
...
The data on the number of movie theaters is unavailable in our database before 1992. However, we hypothesize that part of the decline in the number of seats is due to the closure of movie theaters. Nevertheless, this hypothesis is not sufficient to understand the evolution of the number of seats during this quarter-century.

What might be surprising is that, during the same period when the number of seats diminished, the number of screens had a different trajectory.

Until 1971, the number of seats and screens followed the same downward trend. However, while the number of seats plummeted, the number of screens stabilized and then increased between 1972 and 1985. The graph below highlights these elements. Using a 1966 base index of 100 for both variables allows us to focus on their evolution, setting aside the scale effect. The index of the number of seats almost halved from 1972 to 1985. In parallel, the index of the number of screens was more than 20 points higher in 1985 than what was observed in 1972. It was even 5 points above the index in 1966. Likewise, despite the fact that the number of screens experienced a period of decline between 1986 and 1992, a significant gap exists between the two indexes in 1992.
...
The contradictory trends in the number of seats and screens can be comprehended through a structural change in the movie theater market, as explained by Thiolliere and Ralite (2003). Before the late '60s, a significant portion of the movie theater market was dominated by single-screen theaters with a substantial number of seats. However, this changed in the second half of the 20th century. There was a transition from one-room movie theaters to halls with fewer seats but a plural number of screens.

According to Thiolliere and Ralite, this structural shift was driven by the conversion of former one-room theaters into multi-screen setups, and newly constructed theaters adopted this multiroom model. The emergence of multi-screen theaters allowed the industry to enhance its exploitation capabilities, providing more choices for consumers. These new theaters outperformed their one-room predecessors, leading to the gradual disappearance of the latter.

This shift is reflected in the ratio of chairs to screens, serving as an indicator of the size of the screening room in terms of seats. This ratio consistently decreased since 1969, ultimately halving by 1992.
...
In many movies, the hero often experiences a dramatic downfall, only to make a triumphant comeback. This classic storyline not only captivates audiences on the big screen but also mirrors the history of the movie theater industry. The 25-year period between 1993 and 2019 marks a notable resurgence for the market.

Attendance surged from 130 million to surpass the symbolic milestone of 200 million. While this level remains lower than the peak observed during the "golden age," the upturn is significant. There was a growth of `r Growth_rate_1993_2019`% over the entire period, with an average annual growth rate of `r Growth_rate_1993_2019`% during the period and an average growth of `r Av_Growth_rate_1993_2019`%.

However, it's crucial to acknowledge the impact of the COVID-19 crisis, which dealt a substantial blow to the industry. The aftermath of this crisis has left a lasting effect, and the industry is still on the path to recovery. Our analysis will not primarily focus on these years.
...
As mentioned earlier, it is crucial to acknowledge that growth is not uniform across all departments. While nearly every department witnessed a period of increase, the western part of the territory, for instance, seems to have undergone a more robust and earlier upturn.
...
Numerous scientific papers and institutional reports agree that this revival is due to the development of multiplexes (Blanchard, 2014 ; CNC, 2013; Muhlberger, 2013; Thiolliere and Ralite, 2003). Theses structures would have reignited consumer interest in movie theaters.
...
Data on the number of movie theaters were not available in the database before 1992. The existing data seem to reveal three phases:

-   a period of growth in the 90s

-   a decline in the number of movie theater between 2000 and 2014

-   and, a small revival between 2014 and 2020.

However, when observing the number of movie theaters, excluding multiplexes, a different story unfolds. The graph below shows the evolution of the number of movie theaters, both with and without multiplexes. It appears that multiplexes accounted for most of the growth in the 1990s. The number of movie theaters has even plummeted since 2000 when multiplexes are excluded from the count. Nevertheless, it is important to note that our analysis is limited by an element whose significance remains uncertain. Our data do not allow us to distinguish between the outright closure of movie theaters and the transformation of former multiroom movie theaters into multiplexes.
...
Et_graph <- Etablissement_nat %>% 
  ...
  labs(title = "Number of movie theater in France between 1992 and 2022", x="Year", y="Number of movie theater")+
  geom_line(linewidth = 0.75)+
  theme_classic() +
...
During this period, the proportion of multiplexes within the overall number of movie theaters increased, experiencing a gain of 9.5 percentage points from 1993 to 2018. It's noteworthy that although multiplexes represent only a tenth of all movie theaters, their influence on total attendance is substantial. In 2012, multiplexes contributed to over half of the total entries, as highlighted in Blanchard's study (2014).
...
The number of seats has also experienced a resurgence. While this growth doesn't fully offset the historic decline witnessed in the second half of the 20th century, the increase is substantial compared to the levels observed in 1993. In total, the number of seats has grown by `r Se_diff_rate_1993_2022` seats, reflecting a growth rate of `r Se_Growth_rate_1993_2022` %.
...
However, despite the observed increase in seats, screens are growing at a faster rate than seats. This suggests that individual movie rooms are becoming smaller over time, with approximately 180 seats per screen today compared to 500 in 1966.
...
An interesting observation is that the be diversity in the departmental trajectories exist also in the evolution of the number of seats. The plot below contains an index representing each department's evolution in the total national number of seats. Precisely, each year indicates the growth rate of their share in the total national level aggregate relatively to the observed level in 1966. In other words, it can be understood as a dynamic index of concentration. The graph indicates that France's north-east has seen a decline of its share over the last 60 years. On the other side, some departments in Ile-de-France and the south-east experienced a positive trend. It is also true, but to a lesser extent, for the west of the Hexagon.
...
```{r fig.width=10, fig.height=12}
#we look at the temporal evolution of the share of the department's capacity compared to the national capacity to see where the "supply" is concentrated over time.
map_data3 <- merge(france_shape, seats_dep, by = "departement_code")

ggplot(map_data3, aes(fill = per_seats_dep_change)) +
  geom_sf() +
  scale_fill_viridis_c(option = "C") + 
  labs(title = "Evolution of the relative concentration of the offer of seats by department in France", fill = "Growth rate") +
  theme_classic() +
  annotation_scale()+
...
<br>
This heterogeneity seems partly linked to the evolution of population. comparable index, focusing on the population share, underscores a similar concentration for departments around Paris and in the southeast of France. Nevertheless, the decrease in the share of the North-East is more important in the seat index than to the population one.
...
```{r fig.width=10, fig.height=12}
#We loot at the temporal evolution of each department's share in the population to identify where the population is concentrated over time.

map_data4 <- merge(france_shape, pop_dep, by = "departement_code")

ggplot(map_data4, aes(fill = per_pop_dep_change)) +
  geom_sf() +
  scale_fill_viridis_c(option = "C") + 
  labs(title = "Evolution of the relative concentration of the population by department in France ", fill = "Growth rate") +
  theme_classic() +
  annotation_scale()+
  annotation_north_arrow(style = north_arrow_nautical, location = "tr", which_north = "true")+
  facet_wrap(~year, ncol = 3)+
...
<br> 
The number of screenings per movie theater more than doubled since 1992.
...
Seances_graph <- seance_nat %>%
  ...
  geom_line(color="purple", linewidth = 0.75) +
  labs(title="Number of screenings per movie theater between 1993 and 2022", x="Year", y="Screenings")+
  theme_classic()

ggplotly(Seances_graph, dynamicTicks = TRUE)
```

During the same period, the inflation-adjusted ticket prices tended to decrease. The years 2008 and 2014 stand out as outliers, potentially influenced by the economic context, including the subprime crisis in 2008 and the sovereign debt crisis between 2010 and 2012. However, the downward trend began in the 90s. In nominal terms, ticket prices did not decrease but rather increased. However, the rate of increase was slower than the general price level.
...
With or without adjustment for inflation, revenues have consistently increased over the past thirty years, thanks to the rise in audience numbers. Between 1992 and 2010, revenues grew by 500 million dollars. They also exhibited a peculiar resilience to economic shocks. While small decreases were observed in 2007 at the dawn of the subprime crisis and in 2013 after the sovereign crisis, both were mild and of short duration. This seems to confirm Muhlberger’s work (2013). He studied the relationship between attendance and the state of the economy in France between 1992 and 2012 and found that movie theaters were one of the most resilient cultural activities to shifts in economic conditions.
...
In recent years, French cinema has experienced a notable rise in arthouse films, adding diversity and depth to the cinematic landscape. These artistic and unconventional productions have gained recognition for their unique storytelling and innovative filmmaking techniques. Factors such as increased audience appreciation, support from the film industry, and recognition at prestigious festivals, including Cannes, have propelled arthouse films into the spotlight. The accessibility provided by streaming platforms has further contributed to their popularity, allowing audiences to explore a range of cinematic styles. This shift signifies a growing preference for intellectually engaging storytelling and showcases the dynamic evolution of the French film industry.
...
AE_graph1 <- Attendance_AE_nat %>% 
  ...
  labs(title ="Attendance of arthouse cinema in France", x="Year", y="Attendance")+
  theme_classic()
...
```{r}
growth_revenue_2002_2019 <- calculate_growth_rate(recette_nat, 2002, 2019)

growth_revenue_AE_2002_2019 <- calculate_growth_rate(recette_AE_nat, 2002, 2019)
```

Notably, the surge in arthouse cinema has been accompanied by a significant growth in revenue, experiencing an increase of `growth_revenue_AE_2002_2019`% from 2002 to 2019, compared to `growth_revenue_2002_2019`% for the overall industry.

Arthouse films have demonstrated their commercial viability, attracting audiences with a penchant for more thought-provoking and unconventional narratives. This financial success further underscores the changing landscape of French cinema, where arthouse productions are not only gaining critical acclaim but also proving to be lucrative contributors to the industry's overall revenue.

```{r}
AE_graph3 <-  recette_AE_nat %>% 
  ggplot(aes(x = year, y = recetteAE_tot)) +
  geom_line(color = "skyblue", linewidth = 0.75) + 
  labs(title="Totale revenue of arthouse cinema in France", x="Year", y="Totale Revenue")+
  theme_classic()
...
In 2021, arthouse cinema claimed a remarkable 63% of the French film market, showcasing a significant shift in audience preferences and the enduring popularity of these films. This success not only reinforces the cultural and artistic significance of arthouse productions but also positions them as dominant players in the French cinematic landscape.
...
AE_graph4 <-  Etablissement_AE %>% 
  ...
  geom_line(color = "purple", linewidth = 0.75) + 
    scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  labs(title="Percentage of arthouse cinema in France", x="Year", y="Percentage of Arthouse Cinema")+
  theme_classic()
...
X <- Econ_data_v2 %>% ungroup() %>% select(-departement_code, -year, -entrées)
X_names <- paste(colnames(X), collapse = " + ")
X_Y <- paste("entrées ~ ",X_names)

#OLS Model
ols <- plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="pooling")

#Fixed effect model
f_eff <-plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="within")
...
#Random effect model
r_eff <-plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="random") 
...
f_eff_time <-plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="within", effect = "time")
...
f_eff_2ways <- plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="within",effect = "twoways")
```
...
bptest(as.formula(X_Y), data = Econ_data_v2, studentize=F)
```
...
f_eff_2ways_b90 <- plm(X_Y, data=Econ_data_v2_1, index=c("departement_code", "year"), model="within",effect = "twoways")

#After 1990 (not included)
Econ_data_v2_2<- Econ_data_v2 %>% filter(year > 1990)

f_eff_2ways_a90 <- plm(X_Y, data=Econ_data_v2_2, index=c("departement_code", "year"), model="within",effect = "twoways")
```
...
#N
N_model <- Econ_data_v2 %>% 
  select(departement_code) %>%
  n_distinct()


#T
T_model <- Econ_data_v2 %>% 
  select(year) %>%
  n_distinct()
...
```{r VIF_test}
mvif <-plm(X_Y, data=Econ_data_v2, index=c("departement_code", "year"), model="pooling")
...
## Annex -1 Test on the CSP models for the year before 1990
...
#OLS Model
ols_b90 <- plm(X_Y, data=Econ_data_v2_1, index=c("departement_code", "year"), model="pooling")

#Fixed effect model
f_eff_b90 <-plm(X_Y, data=Econ_data_v2_1, index=c("departement_code", "year"), model="within")
...
#Random effect model
r_eff_b90 <-plm(X_Y, data=Econ_data_v2_1, index=c("departement_code", "year"), model="random") 
...
#Model with time-fixed effects.
f_eff_time_b90 <-plm(X_Y, data=Econ_data_v2_1, index=c("departement_code", "year"), model="within", effect = "time")
...
bptest(as.formula(X_Y), data = Econ_data_v2_1, studentize=F)
```
...
### Annex 2 - Test on the CSP models for the year after 1990
...
# OLS Model
ols_a90 <- plm(X_Y, data=Econ_data_v2_2, index=c("departement_code", "year"), model="pooling")

#Fixed effect model
f_eff_a90 <-plm(X_Y, data=Econ_data_v2_2, index=c("departement_code", "year"), model="within")
...
#Random effect model
r_eff_a90 <-plm(X_Y, data=Econ_data_v2_2, index=c("departement_code", "year"), model="random") 
...
#Model with time-fixed effects.
f_eff_time_a90 <-plm(X_Y, data=Econ_data_v2_2, index=c("departement_code", "year"), model="within", effect = "time")
...
bptest(as.formula(X_Y), data = Econ_data_v2_2, studentize=F)
```
