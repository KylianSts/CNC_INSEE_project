...
#Comparaison the number of seats and screens
 ...
 Equip_nat_b100 <- Equip_nat %>% select(year,Screens_b100,Seats_b100) %>% 
   pivot_longer(cols = c(Screens_b100,Seats_b100),names_to = "Variable", values_to =  "Base_index_100")
 
 ##Graphic
 Se_Sc_graph1 <- Equip_nat_b100 %>% 
   filter(year<1993) %>% 
  ggplot(aes(x=year, y=Base_index_100, color=Variable)) +
  geom_line() +
  scale_color_manual(values = c("red", "brown")) +
  labs(title = "Comparaison of the number of seats and screens (base index 100)", x="Year", y="Number of seats")+
  theme_minimal()
...
```{r}
#Computation of the growth rate between 1993 and 2018
...
```{r}
 M_graph1 <- Multiplex_nat %>% 
  ...
  geom_bar(fill="navy", stat = "identity") + 
   labs(titre="Number of multiplexes in France by year between 1993 and 2018", x="Year", y="Number of multiplexes")+
  theme_minimal()
...
The share of multiplexes in the total number of movie theaters also grew during this year, gaining 9.5 percentage points between 1993 and 2018. Furthermore, if multiplexes may still be a minority in the overall number of cinemas, this is not the case for the total number of attendance. Indeed, they accounted for more than half of the entries in 2012 (Blanchard, 2014).
...
The number of screens rose with the development of multiplexes.
...
The number of seats started also to grow again. This growth is smaller than the significant decline that marked the second haft of the 20th century - c.f. graph below on the 1962-2022 period. However, it is significant regarding the level observed in 1993 -c.f. graph below on the 1993-2018 period. In total, the numver of seats increased of `r Se_diff_rate_1993_2018` seats, representing an increase of `r Se_Growth_rate_1993_2018` %.
