...
```{r}
inflation_fr <- read_excel('Data/inflation_france.xlsx') %>%
  select(-`Inflateur Cumulé`) %>%
  filter(Année > 1965) %>%
  rename(year = Année) %>%
  arrange(year)


cnc_data_clean$recettes_updated <- cnc_data_clean$recettes
data <- cnc_data_clean %>% 

for (i in 1:length(data)) {
  for (j in 1:length(2022 - cnc_data_clean$year[i])) {
    data$recettes_updated[i] <- cnc_data_clean$recettes_updated[i] * (1 + inflation_fr$`Inflation Moyenne Annuelle`[j])
  }
 print(length(2022 - cnc_data_clean$year[i]))
}
...
recette_nat <- cnc_data_clean %>% 
  group_by(year) %>% 
  summarize(recette_tot = sum(recettes_updated, na.rm = TRUE))
#Graphic of the evolution of attendance in time
...
Dep_shp <- st_read(dsn ="Maps/ADMIN-EXPRESS/1_DONNEES_LIVRAISON_2023-12-00156/ADE_3-2_SHP_LAMB93_FXX-ED2023-12-19/DEPARTEMENT.shp")
...
Dep_shp <- Dep_shp %>% left_join(Dep_attb100, by = c("INSEE_DEP" = "departement_code")) %>% filter(year %in% c(1966,1968,1972,1975,1979,1982,1985,1988,1990,1993,1996,1999,2002,2005,2008,2011,2013,2016,2019))
