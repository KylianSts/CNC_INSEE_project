...
We only used basics technics for econometric analysis of panel data. 3 simple and standards models are often used for panel data :
...
An early step consists to analyze which technic fits the most the dataset. The F Test for Individual and/or Time Effects of the plm package (pFtest) enable to arbitrate between the used of a fixed effect model a simple OLS. The decision rule is that if the p-value is below the significance level of 0.05, the fixed effect model is preferred. Here our p-value is, then we will prefered the fixed model.
...
The Haussman test continue our analysis by analysing which of the fixed effect model and the random one is the best fit for the model. Here, our p-value is below 0.05, the random effect model is choosen.
...
##### 2nd step : Test for time-fixed effect

The fixed effect model being chosen, the next step was to identify if the addition of individual or time-fixed effects improve the quality of the model.

We used TORRES-REYNA (2010)'s method which consist to add dummy variable to create our two new models. An alternative methode exist that consist to use the effect argument of the plm package with code such that code ' effect ="time" 'or effect = "individual" '(Wagle, 2021; Croissant et al., 2023; Hanck et al., 2023). However, nous avons du en écarter l'usage parce qu it gave us results that seems incoherent with too big regressor.

With a p value below 0.05, the Brehsch-Pagan test support the hypothesis of the use of time-fixed effect but the Honda test reject it.
...
The Lagrange multiplier test emphasized the paralell use of individual effect.
...
We will then used a two way model that combined time-fixed and individual effect.
...
It can be noted that if it gave us different coefficient, the use of the argument effect arrived at the same conclusion of the usage of a two-way model :
...
```
 ...
 However, the use of the vcovHC function for the sandwich package compute heteroskedasticity-consistent covariance estimators using White and Arellano method (White 1980, 1984; Arellano 1987). Moreover, it can be used to correct simultaneously both heteroskedasticity and serial correlation with the code ' method = "arellano" '.
 
##### Test for cross-sectional dependence 
A last issue is common with panel data, and ours is not an exception. This issue is the cross-sectiona dependance. As we can see on the Breusch-Pagan and Pesaran tests below, our model suffers of this issue Beck and Katz method. To do so, we will finally not used the vcovHC function but do a similar work with the cvocBK which also tackles cross-sectional dependence
