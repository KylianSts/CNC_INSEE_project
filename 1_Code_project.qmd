...
insee_econ <- insee_econ  %>%  select(-X) %>% filter(nchar(DEP_RES_21) != 3) %>%  rename(departement_code = DEP_RES_21, year =  AN_RECENS)
...
f_eff_time<-plm(entrées~ a_less_than_6 + a_6_to_24 + a_25_to_42 + a_43_to_59 + a_60_and_more + factor(year), data=Econ_data_v2, index=c("departement_code", "year"), model="within")
