...
t in the cinema market. Blanchard ([2014](https://www.cairn.info/revue-migrations-societe-2014-1-page-153.htm)) argues that these facilities "have contributed to the resurgence of attendance, notably by making the cinema more accessible to sub
...
```{r}
#Aggregation of the number of chairs and screens at the national level
Equip_nat <- cnc_data_clean %>% 
  group_by(year) %>% 
  summarize(Screens = sum(écrans, na.rm = TRUE), Chairs=sum(fauteuils, na.rm=TRUE))
...
Equip_nat <- Equip_nat %>%  mutate(Screens_b100=Screens/Screens[year==1966], Chairs_b100=Chairs/Chairs[year==1966])



#Evolution of the number of chairs
Ch_graph1 <- Equip_nat %>% 
  ggplot(aes(x=year, y=Chairs)) +
  geom_line(color="brown")+
  labs(title = "Number of chairs", x="Year", y="Number of chairs")+
  theme_minimal()

F_Ch_graph1 <- ggplotly(Ch_graph1)


F_Ch_graph1

#Evolution of the number of screens
Sc_graph1 <- Equip_nat %>% 
  ggplot(aes(x=year, y=Screens)) +
  geom_line(color="red")
  labs(title = "Number of screens by year", x="Year", y="Number of chairs")+
  theme_minimal()

Sc_graph1$yaxis$title <- list(tickformat = ",.3s")
F_Sc_graph1 <- ggplotly(Sc_graph1)


F_Sc_graph1

#Comparaison the number of chairs and screens
Ch_Sc_graph1 <- Equip_nat %>% 
  ggplot(aes(x=year)) +
  geom_line(aes(y=Chairs_b100), color="brown") +
  geom_line(aes(y=Screens_b100), color="red")
  labs(title = "Comparaison of the number of chairs and screens (base index 100)", x="Year", y="Number of chairs")+
  theme_minimal()

Ch_Sc_graph1$yaxis$title <- list(tickformat = ",.3s")
F_Ch_Sc_graph1 <- ggplotly(Ch_Sc_graph1)
...
#Ratio screen/chair
Equip_nat <- Equip_nat %>% 
  mutate(Chairs_by_screens = Chairs/Screens)


Ch_Sc_graph2 <- Equip_nat %>% 
  ggplot(aes(x=year,y=Chairs_by_screens))+
  geom_bar(stat="identity")+
  labs(title = "Ratio of Chairs by Screens in France by Year", x="Year", y="Chairs by Screen")+
  theme_minimal()

F_Ch_Sc_graph2 <- ggplotly(Ch_Sc_graph2)
...
F_Att_graph8 <-ggplotly(Att_graph8)

F_Att_graph8
```

```{r}
 Att_graph10 <- multiplex_nat %>% 
  filter(year >1992) %>% 
...
F_Att_graph10 <-ggplotly(Att_graph10)
