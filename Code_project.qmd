...
cnc_data_clean %>% 
  group_by(year) %>% 
  summarize(across( where(is.numeric),list(Mean = ~mean(., na.rm = TRUE), Median = ~median(., na.rm = TRUE)))) %>% t() %>% 
...
insee_data %>%
  ...
  mutate(across(everything(), ~prettyNum(., big.mark = " "))) %>% pivot_longer(everything(),names_to = "Variable", values_to="N.") %>% 
...
data_clean |> group_by(year) |>  summarise(nb_value = n())
```

## Research question

The goal of this project is to answer at a dual challenge.

The first aspect of our investigation is to understand the dynamic of French movie theaters over the last 50 years.

Questions such as "How evolves the number of venues?"  or "What is the evolution of attendance levels ?" will be at the core of this paper. To be more precise, the goal of the project is, therefore, to understand how movies theater have evolved in terms of volume (i.e. number of location), size (i.e. seats and screens), attendance and, revenue. The use panel data allows us to try to grasp not only the overall temporal trend of our subject but also to observe potential cross-sectional differences. Indeed, one of our key objectives is to comprehend whether there are differences in trajectories and characteristics among French departments.
