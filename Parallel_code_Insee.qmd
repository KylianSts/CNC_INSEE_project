...
However we do not need so many combinaitions. Indeed, among these variables we find :

-   some variables wich has many levels but are useless for us;

-   but also the variable AGE_REV, a variable that give us the age that can have an inhabitant. A variable wichi is important but has to many levels (a level by age).
...
```{r}
insee <- insee |> 
  mutate(AGE_GROUP = case_when( AGE_REV < 15 ~ "<15", between(AGE_REV, 15, 29) ~ "15-29", between(AGE_REV, 30, 44) ~ "30-44", between(AGE_REV, 45, 59) ~ "45-59", between(AGE_REV, 60, 74) ~ "60-74", AGE_REV > 75 ~ "75 and more", TRUE ~ NA_character_)) 

Insee_cleanv3 <- insee |> group_by(AN_RECENS,DEP_RES_21,AGE_GROUP,TYP_ACT,CSP) |> summarize(test = sum(POND, na.rm = TRUE)) rm(insee)
```
